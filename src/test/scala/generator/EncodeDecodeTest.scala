...
class EncodeDecodeTest extends WordSpec with Matchers {
  ...
  "Decoder method isExpired" must {
    "check date in key for expiration and return true" in {
    ...
    "check date in key for expiration and return false" in {
  ...
  "Decoder method verify" must {
    "return false when date is invalid" in {
      val date = LocalDate.of(2000, 11, 11)
    ...
    "return false when domain is invalid" in {
      val date = LocalDate.of(2000, 11, 11)
    ...
    "return true when all data is fine" in {
      val date = LocalDate.of(2020, 11, 11)
  ...
  "Encoder method encode" must {
    "return equals values for the same input" in {
